...
public class AINotebook {
	...
	public AINotebook(Player player, List<Player> players, List<Room> rooms, List<Suspect> suspects, List<Weapon> weapons) {
		aiPlayer = player;
		playersWithUnknownCards = new ArrayList<Player>(players);
		playersWithUnknownCards.remove(aiPlayer);
		newInformationList = new LinkedList<NewInformation>();
		
		roomToAccuse = null;
		suspectToAccuse = null;
		weaponToAccuse = null;
		
		roomNotebookEntries = new HashMap<Room, AINotebookEntry>();
		for (Room room : rooms)
			roomNotebookEntries.put(room, new AINotebookEntry(playersWithUnknownCards));
		
		suspectNotebookEntries = new HashMap<Suspect, AINotebookEntry>();
		for (Suspect suspect : suspects)
			suspectNotebookEntries.put(suspect, new AINotebookEntry(playersWithUnknownCards));
		
		weaponNotebookEntries = new HashMap<Weapon, AINotebookEntry>();
		for (Weapon weapon : weapons)
			weaponNotebookEntries.put(weapon, new AINotebookEntry(playersWithUnknownCards));
		
		suggestedAnswers = new ArrayList<SuggestedAnswer>();
	...
	private void countCards() {
		...
		while (playerIter.hasNext()) {
			...
			if (knownCards < player.getCards().size()) {
				Set<Room> possibleAnsweredRooms = new HashSet<Room>();
				Set<Suspect> possibleAnsweredSuspects = new HashSet<Suspect>();
				Set<Weapon> possibleAnsweredWeapons = new HashSet<Weapon>();
