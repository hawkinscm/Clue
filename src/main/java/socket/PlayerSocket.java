...
/**
 * A participant-connected main.java.socket that is used to communicate back and forth between the host and a participating network player.
 */
public class PlayerSocket extends Socket {
	...
	public PlayerSocket(int portIndex) {
		...
		try {
			...
			String message = "<html> Another application is using the desired main.java.socket: " + (BASE_PORT + portIndex) + ". <br>";
	...
	/**
	 * Returns the connection main.java.socket port number used by this Player Socket.
	 * @return the connection main.java.socket port number used by this Player Socket
	 */
	 ...
	 */
	public String connect(String defaultName) {
		try {
			serverSocket.setSoTimeout(0);
			socket = serverSocket.accept();
			reader = new BufferedReader(new InputStreamReader(socket.getInputStream()));
			writer = new PrintWriter(socket.getOutputStream());
			writer.println(defaultName);
			writer.flush();
			return reader.readLine();
		} 
		catch (IOException ex) {
			if (ex instanceof SocketException && ex.getMessage().equals("main.java.socket closed"))
				return null;
	...
	/**
	 * Closes and reconnects the main.java.socket.
	 * @throws IOException on any main.java.socket/IO error
	 */
	...
	/**
	 * Returns whether or not the server main.java.socket is connected.
	 * @return true if this main.java.socket is connected; false otherwise
	 */
