...
public class Board extends Observable {
	...
	public ArrayList<DisplayTile> getTiles() {
		ArrayList<DisplayTile> tiles = new ArrayList<>(height * width);
		for (int row = 0; row < height; row++)
			tiles.addAll(Arrays.asList(boardTiles[row]).subList(0, width));
		return tiles;
	}
	...
	public LinkedList<DisplayTile> getRoomTiles(Room room) {		
		LinkedList<DisplayTile> roomTiles = new LinkedList<>();
		if (room == null)
			return roomTiles;
	...
	public List<DisplayTile> getFreeExitTiles(Room room) {
		List<DisplayTile> exitTiles = new LinkedList<>();
			...
			for (Direction direction : Direction.values()) {
				if (roomTile.hasDoor(direction)) {
					...
					if (exitTile.isRoomTile())
						...
						isFree = !exitTile.isPassage() || !exitTile.getPassageConnection().hasSuspect();
	...
	public DisplayTile moveSuspectToRoom(Suspect suspect, Room room) {
		...
		List<DisplayTile> leastOccupiedTiles = new LinkedList<>();
	...
	public char getNextUnusedPassageLetter() throws NoSuchElementException {
		LinkedList<Character> passageLetters = new LinkedList<>();
	...
	public boolean validate(List<Room> rooms, List<Suspect> suspects) {
		LinkedList<String> invalidMessages = new LinkedList<>();
		for (Room room : rooms) {
		...
		StringBuilder message = new StringBuilder("<html>This is not a completed, playable Clue Board for the following reason(s):");
		for (String invalidMessage : invalidMessages)
			message.append("<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;").append(invalidMessage);
		message.append("</html>");
		Messenger.warn(message.toString(), "Clue Board Validation");
		return false;
	}
